...
Write-Output "Commit: $($ciRun.head_sha) - $($ciRun.display_title)"
Write-Output "Status: $($ciRun.status) / $($ciRun.conclusion)"
$jobs = Invoke-RestMethod "https://api.github.com/repos/Dara8091/fut-invest/actions/runs/$($ciRun.id)/jobs"
foreach ($j in $jobs.jobs) {
    $c = if ($j.conclusion) { $j.conclusion } else { '...' }
    Write-Output "  $($j.name): $c"
}
